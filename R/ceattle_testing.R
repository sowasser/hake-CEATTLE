...
hake_data <- read_data(file = "data/hake_intrasp_noNBA.xlsx")
...
# Run new model
new_model <- run_CEATTLE(data = hake_data, 
                         M1 = 1, 
                         prior = TRUE, 
                         init = NULL, 
                         msm = 1, 
                         estMode = 0)
new_model$fit  # check convergence
new_model$model$quantities$M1

# Compare to base (publication) model
load("models/ms_priorM1.Rdata")

plot_biomass(Rceattle = list(new_model$model, ms_priorM1$model), 
             model_names = c("New Model", "Base Model"),
             incl_proj = TRUE, 
             add_ci = TRUE)
plot_ssb(Rceattle = list(new_model$model, ms_priorM1$model), 
         model_names = c("New Model", "Base Model"),
         incl_proj = TRUE, 
         add_ci = TRUE)
...
# all_yrs <- ms_model$model$data_list$styr:ms_model$model$data_list$projyr
# hind_end <- 2019
# assess_yr = "2020"
# max_age <- ms_model$model$data_list$nages
# 
