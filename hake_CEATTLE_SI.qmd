...
![**Figure S.2**: Age vs. length (cm) from the 2020 hake stock assessment, the ages fitted to California Current Trophic Database (CCTD) predators and prey using a von Bertalanffy growth curve parameterized using the length-at-age data from the hake acoustic-trawl survey, and ages from the hake acoustic-trawl survey data.](./plots/diet/growth_curve.png)

![**Figure S.3**: Number of hake prey (sample size), observed proportion of cannibalism by weight in hake stomachs, and the predicted diet proportion by weight from the cannibalism CEATTLE model by predator age, as rows, prey ages, and year.](./plots/CEATTLE/predicted_diet_comparison.png)
